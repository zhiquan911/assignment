...
use sp_runtime::{
	...
	traits::{BlockNumberProvider, IdentifyAccount, StaticLookup},
...
// We are fetching DOT new price fro this remote api
const HTTP_REMOTE_DOT_PRICE: &str = "https://api.coincap.io/v2/assets/polkadot";
...
pub mod crypto {
	...
	// 账户ID转公钥接口
	pub trait AccountToPublic<T: SigningTypes, Public> {
		fn account_into_public(a: T::AccountId) -> Option<T::Public>;
	}
	...
	impl<T: SigningTypes> AccountToPublic<T, MultiSigner> for TestAuthId {
		// type Public = S::Public;
		fn account_into_public(a: T::AccountId) -> Option<T::Public> {
			let pk = a.using_encoded(|b| sp_core::sr25519::Public::decode(&mut b.clone())).ok()?;
			let mpk = MultiSigner::Sr25519(pk);
			let spk = mpk.using_encoded(|b| T::Public::decode(&mut b.clone())).ok()?;
			Some(spk)
		}
	}

	impl<T: SigningTypes> AccountToPublic<T, sp_core::sr25519::Public> for TestAuthId {
		// type Public = S::Public;
		fn account_into_public(a: T::AccountId) -> Option<T::Public> {
			let pk = a.using_encoded(|b| sp_core::sr25519::Public::decode(&mut b.clone())).ok()?;
			let spk = pk.using_encoded(|b| T::Public::decode(&mut b.clone())).ok()?;
			Some(spk)
		}
	}
}
...
pub mod pallet {
	...
	pub trait Config: frame_system::Config + CreateSignedTransaction<Call<Self>> {
		...
		type AuthorityId: AppCrypto<Self::Public, Self::Signature>
			+ AccountToPublic<Self, Self::Public>;
	}
	...
	#[pallet::storage]
	#[pallet::getter(fn price_provider)]
	pub type PriceProvider<T: Config> = StorageValue<_, T::AccountId, OptionQuery>;

	#[pallet::event]
	#[pallet::generate_deposit(pub(super) fn deposit_event)]
	pub enum Event<T: Config> {
		NewNumber(Option<T::AccountId>, u64),
		NewPrice((u64, Permill), Option<T::AccountId>),
		PriceProviderSet(T::AccountId),
	}
			...
			match call {
				...
				Call::submit_price_unsigned_with_signed_payload(ref payload, ref signature) => {
					...
					if let Some(price_provider) = Self::price_provider() {
						// 交易发送者
						let sender = SignedPayload::<T>::public(payload).into_account();
						// 判断发送者是否价格提供者
						if sender != price_provider {
							return InvalidTransaction::BadProof.into()
						}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_price_provider(
			...
			// This ensures that the function can only be called root.
			ensure_root(origin)?;
			let who = T::Lookup::lookup(provider.clone())?;
			PriceProvider::<T>::put(who.clone());
			Self::deposit_event(Event::PriceProviderSet(who));
			Ok(())
		}
	}
}
...
impl<T: Config> Pallet<T> {
	...
	// AccountId to Public
	fn account_to_public(acc: T::AccountId) -> Result<T::Public, Error<T>> {
		T::AuthorityId::account_into_public(acc).ok_or(Error::<T>::DecodeError)
	}
	...
	fn fetch_price_info() -> Result<(), Error<T>> {
		...
		let price = Self::get_remote_new_price().map_err(|_| Error::<T>::HttpFetchingError)?;
		// 取得价格提供者的账户
		let price_provider = Self::price_provider().ok_or(Error::<T>::PriceProviderNotSet)?;
		// 账户转公钥
		let filter_account = Self::account_to_public(price_provider)?;
		// 过滤本地密钥只取filter_account账户的密钥对
		let account =
			Signer::<T, T::AuthorityId>::any_account().with_filter(vec![filter_account.clone()]);
		// 采用【不签名但具签名信息的交易】
		// 因为很多时候签名交易意味签名者需要为该交易付手续费。我们通过Root设置了价格提供者账户，只有该账户才能提交DOT的最新价格。
		// 所以我们只需验证发送者是否该账户，并验证他的消息签名，就可以免手续费更新价格，又能避免恶意账户刷价格。
		let (_, result) = account
			.send_unsigned_transaction(
				|account| PricePayload { price, public: account.public.clone() },
				Call::submit_price_unsigned_with_signed_payload,
			)
			.ok_or(Error::<T>::NoLocalAcctForSigning)?;
		result.map_err(|()| Error::<T>::OffchainSignedTxError)?;

		Ok(())
	}

	fn get_remote_new_price() -> Result<(u64, Permill), http::Error> {
		let deadline = sp_io::offchain::timestamp().add(Duration::from_millis(2_000));

		let request = http::Request::get(HTTP_REMOTE_DOT_PRICE);
	...
	fn parse_price(price_str: &str) -> Option<(u64, Permill)> {
		if let Ok(v) = serde_json::from_str::<serde_json::Value>(price_str) {
			// {
			// 	"data": {
			// 		"id": "polkadot",
			// 		"rank": "10",
			// 		"symbol": "DOT",
			// 		"name": "Polkadot",
			// 		"supply": "1074143042.6177700000000000",
			// 		"maxSupply": null,
			// 		"marketCapUsd": "29160911669.4024505600420485",
			// 		"volumeUsd24Hr": "784653858.4270016195196893",
			// 		"priceUsd": "27.1480710784431881",
			// 		"changePercent24Hr": "1.3036924674520133",
			// 		"vwap24Hr": "27.3136800049154390",
			// 		"explorer": "https://polkascan.io/polkadot"
			// 	},
			// 	"timestamp": 1642254520198
			// }
			if let Some(price_usd) = v["data"]["priceUsd"].as_str() {
				let mut price_int = 0u64;
				let mut price_float = Permill::default();
				let split = price_usd.split(".");
				let mut i = 0u32;
				for x in split {
					if i == 0 {
						price_int = x.parse::<u64>().unwrap_or(0u64);
					} else if i == 1 {
						// Permill is 1,000,000, decimals is 6
						let max_decimals = x.len().min(6);
						let decimals_part = x[0..max_decimals].parse::<u32>().unwrap_or(0u32);
						price_float = Permill::from_parts(decimals_part);
					}

					i = i + 1;
				}
				Some((price_int, price_float))
			} else {
				None
			}
		} else {
	...
	fn add_price(maybe_who: Option<T::AccountId>, price: (u64, Permill)) {
